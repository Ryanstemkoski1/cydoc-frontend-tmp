...
export default class FamilyHistoryContent extends Component {
        ...
        if (!isPreview) {
            ...
            for (let response_index in response_choice) {
                ...
                if (condition_index === -1) {
                    let index = Object.keys(values).length.toString();
                    values[index] = {
                        Condition: response,
                        Yes: false,
                        No: false,
    ...
    render() {
        ...
        let conditions = Object.keys(this.context['Family History']).map(
            (value) => this.context['Family History'][value]['Condition']
        );
        let index_dict = {};
        if (this.props.response_choice) {
            for (let condition_index in list_values) {
                let condition = list_values[condition_index];
        ...
        return mobile ? (
            <>
                <GridContent
                    ...
                    rows={listItems}
                    question_type={
                        this.props.response_choice ? 'hpi' : 'add_row'
                    }
