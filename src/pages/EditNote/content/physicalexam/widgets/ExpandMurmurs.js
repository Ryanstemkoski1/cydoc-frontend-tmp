...
class ExpandMurmurs extends Component {
    ...
    constructor(props, context) {
        ...
        this.state = {
            pe_rows0: [
                ['aortic stenosis', 'best heard RUSB', 'radiation to carotids'],
                [
                    'pulmonary stenosis',
                    'best heard LUSB ',
                    'radiation to left clavicle',
                ],
                ['atrial septal defect', 'best heard LUSB'],
                ['HOCM', 'best heard LLSB', 'incr w Valsalva'],
            ],
            pe_rows1: [
                ['mitral regurg', 'best heard apex', 'radiation to precordium'],
            ...
            pe_rows2: [
                ['mitral prolapse', 'systolic click', 'incr with Valsava'],
                ['physiologic'],
            ],
            pe_rows3: [
                [
    ...
    onButtonClick(event, data) {
        ...
        if (
            this.context['Physical Exam'].widgets['Murmurs'][this.props.ind][
                'systdiast'
            ] === 'systolic'
        ) {
            if (this.state.pe_rows0[0].some((x) => x === data.condition)) {
                values.widgets[this.props.type][data.pe_index][
                    'cresdecres'
                ].push(data.condition);
            } else if (
                this.state.pe_rows0[1].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index][
                    'cresdecres'
                ].push(data.condition);
            } else if (
                this.state.pe_rows0[2].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index][
                    'cresdecres'
                ].push(data.condition);
            } else if (
                this.state.pe_rows0[3].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index][
                    'cresdecres'
                ].push(data.condition);
            } else if (
                this.state.pe_rows1[0].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index][
                    'pansystolic'
                ].push(data.condition);
            } else if (
                this.state.pe_rows1[1].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index][
                    'pansystolic'
                ].push(data.condition);
            } else if (
                this.state.pe_rows1[2].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index][
                    'pansystolic'
                ].push(data.condition);
            } else if (
                this.state.pe_rows2[0].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index]['other'].push(
                    data.condition
                );
            } else if (
                this.state.pe_rows2[1].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index]['other'].push(
                    data.condition
                );
            ...
            if (this.state.pe_rows3[0].some((x) => x === data.condition)) {
                values.widgets[this.props.type][data.pe_index]['decres'].push(
                    data.condition
                );
            } else if (
                this.state.pe_rows3[1].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index]['decres'].push(
                    data.condition
                );
            } else if (
                this.state.pe_rows3[2].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index]['decres'].push(
                    data.condition
                );
            } else if (
                this.state.pe_rows3[3].some((x) => x === data.condition)
            ) {
                values.widgets[this.props.type][data.pe_index]['decres'].push(
                    data.condition
                );
    ...
    render = () => {
        ...
        if (
            ...
            let values = this.context['Physical Exam'].widgets[this.props.type];
