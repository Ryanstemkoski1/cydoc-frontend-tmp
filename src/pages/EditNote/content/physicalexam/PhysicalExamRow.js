...
export default class PhysicalExamRow extends React.Component {
    ...
    handleDropdownChange = (e, { value }) => {
        ...
        let oldValues = this.state.dropdownSelected.filter(
            (v) => !value.includes(v)
        );
        let toggleCallback = this.props.row.needsRightLeft
            ? this.props.handleLRToggle.bind(this, null)
    ...
    generateButtons = ({
            ...
            buttons = findings.map((finding, index) => {
                return (
                    <LRButton
                        ...
                        active={
                            this.context['Physical Exam'][this.props.group][
                                finding
                            ].center
                        }
                        toggle={
                ...
                return (
                    <Button
                        ...
                        onClick={(e, { name, active }) =>
                            this.props.handleToggle(name, !active)
                        }
                        className={'spaced-buttons'}
                    />
                );
        ...
        if (includeSelectAll) {
            return (
                <SelectAllButton handleClick={SelectAllCallBack}>
                    {buttons}
                </SelectAllButton>
            );
        } else {
