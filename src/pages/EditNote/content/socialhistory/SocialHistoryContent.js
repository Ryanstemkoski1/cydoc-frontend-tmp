...
export default class SocialHistoryContent extends React.Component {
    ...
    constructor(props) {
        ...
        this.handleInterestedToggleButtonClick = this.handleInterestedToggleButtonClick.bind(
            this
        );
        this.handleTriedToggleButtonClick = this.handleTriedToggleButtonClick.bind(
            this
        );
        this.handleSocialHistoryChange = this.handleSocialHistoryChange.bind(
            this
        );
        this.handleSecondaryFieldsChange = this.handleSecondaryFieldsChange.bind(
            this
        );
        this.generateSecondaryFieldRows = this.generateSecondaryFieldRows.bind(
            this
        );
        this.substanceUseContentHeader =
            SOCIAL_HISTORY.SUBSTANCE_USE_CONTENT_HEADER;
    ...
    handleToggleButtonClick(event, data) {
        ...
        for (let response_index in responses) {
            ...
            if (data.title !== response)
                values[data.condition][response] = false;
        }
        this.context.onContextChange('Social History', values);
    }
    ...
    handleInterestedToggleButtonClick(event, data) {
        ...
        const prevState =
            values[data.condition]['InterestedInQuitting'][data.title];
        values[data.condition]['InterestedInQuitting'][data.title] = !prevState;
        for (let response_index in responses) {
            let response = responses[response_index];
            if (data.title !== response)
                values[data.condition]['InterestedInQuitting'][
                    response
                ] = false;
        }
        this.context.onContextChange('Social History', values);
    ...
    handleTriedToggleButtonClick(event, data) {
        ...
        for (let response_index in responses) {
            let response = responses[response_index];
            if (data.title !== response)
                values[data.condition]['TriedToQuit'][response] = false;
        }
        this.context.onContextChange('Social History', values);
    }
    ...
    generateSecondaryFieldRows() {
        return this.secondaryFields.map((label, index) => (
                ...
                <Segment>
                    <Tobacco
                        ...
                        onInterestedButtonClick={
                            this.handleInterestedToggleButtonClick
                        }
