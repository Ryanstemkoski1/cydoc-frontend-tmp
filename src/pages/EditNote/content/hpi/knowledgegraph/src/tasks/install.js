...
/*
   ...
   * (NPM) Install - Will ask for where to put src (outside pm folder)
   * (NPM) Upgrade - Will look for src install, copy over files and update if new version
...
var
  currentConfig = requireDotFile('src.json'),
...
// run update scripts if src.json exists
if(currentConfig && manager.name === 'NPM') {
  ...
  if( fs.existsSync(updatePaths.definition) ) {
    ...
    if(currentConfig.version !== release.version) {
      ...
      // update version number in src.json
...
gulp.task('run setup', function() {
  ...
  // If auto-install is switched on, we skip the configuration section and simply reuse the configuration from src.json
...
gulp.task('create install files', function(callback) {
  ...
  gulp.task('create src.json', function() {
    ...
    if( fs.existsSync(installPaths.config) ) {
      console.info('Extending config file (src.json)', installPaths.config);
      ...
      console.info('Creating config file (src.json)', installPaths.config);
