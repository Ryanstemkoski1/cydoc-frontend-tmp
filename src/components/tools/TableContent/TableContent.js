...
export default class TableContent extends Component {
    ...
    constructor(props, context) {
        ...
        for (let i = 0; i < props.values.length; i++) {
            ...
            isInvalidYear.push(
                startYear !== '' &&
                    (startYear < 1900 || startYear > this.currentYear)
            );
        }
        this.state = {
        ...
        for (let i = 0; i < nums.length; i++) {
            ...
            switch (name) {
                case 'surgical history': {
                    ...
                    titleContent = (
                        <Form className='inline-form'>{mainInput}</Form>
                    );
                    break;
                }
                case 'medication': {
            ...
            panels.push({
                ...
                onTitleClick: (event) => {
                    if (
                        ...
                        (event.target.placeholder === 'Inciting Incident' &&
                            event.target.value === '')
                    ) {
