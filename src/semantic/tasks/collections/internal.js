  ...
  ---------------*/
    ...
    gulp.task('package compressed css', function () {
        return gulp
            ...
            .pipe(replace(assets.uncompressed, assets.packaged))
            .pipe(
                concatCSS(filenames.concatenatedMinifiedCSS, settings.concatCSS)
            )
            .pipe(gulpif(config.hasPermission, chmod(config.permission)))
    ...
    if (config.rtl) {
        gulp.task('package uncompressed rtl css', function () {
            return gulp
                ...
                .pipe(replace(assets.uncompressed, assets.packaged))
                .pipe(
                    concatCSS(filenames.concatenatedRTLCSS, settings.concatCSS)
                )
                .pipe(gulpif(config.hasPermission, chmod(config.permission)))
                .pipe(header(banner, settings.header))
                .pipe(gulp.dest(output.packaged))
        ...
        gulp.task('package compressed docs css', function () {
            return gulp
                ...
                .pipe(
                    concatCSS(
                        filenames.concatenatedMinifiedCSS,
                        settings.concatCSS
                    )
                )
                .pipe(minifyCSS(settings.concatMinify))
