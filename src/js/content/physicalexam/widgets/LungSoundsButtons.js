...
export default class LungSoundsButtons extends Component {
    ...
    onPopupClick(event, data) {
        ...
        values["Lungs"][this.props.lung_lobe][data.children[1]] = true 
    ...
    onClick(event, data) {
        ...
        var values = this.context["Physical Exam"]
        var prevState = values["Lungs"][this.props.lung_lobe][data.children]
        values["Lungs"][this.props.lung_lobe][data.children] = !prevState
        if (more_options.includes(data.children) && prevState) delete values["Lungs"][this.props.lung_lobe][data.children]
        this.context.onContextChange("Physical Exam", values);
    }
    ...
    render() {
        ...
        var more_options_buttons = more_options.map(
            item => <Button 
                        ...
                        color={this.context["Physical Exam"]["Lungs"][this.props.lung_lobe][item] ? 'grey': ''} 
        ...
        for (var lung_button in lung_button_dict) {
            lung_buttons.push(
                <Button
                    ...
                    color={this.context["Physical Exam"]["Lungs"][this.props.lung_lobe][lung_button] ? 'grey' : ''}
