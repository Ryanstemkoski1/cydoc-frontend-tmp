...
var when = {
  ...
  // install
  hasConfig: function() {
    return requireDotFile('src.json', process.cwd());
  },
...
module.exports = {
  ...
  // detect whether there is a src.json configuration and that the auto-install option is set to true
  ...
  source: {
    config       : './src.json.example',
    definitions  : './src/definitions',
    gulpFile     : './gulpfile.js',
    lessImport   : './src/src.less',
    site         : './src/_site',
    tasks        : './tasks',
    ...
    setup: [
      {
        type: 'list',
        name: 'overwrite',
        message: 'It looks like you have a src.json file already.',
        when: when.hasConfig,
        choices: [
          {
    ...
    cleanup: [
      {
        ...
        choices: [
          {
            name: 'Yes (re-install will require redownloading src).',
            value: 'yes'
          },
          {
            name: 'No Thanks',
